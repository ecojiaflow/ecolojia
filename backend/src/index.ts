  ...
  try {
    ...
    const product = await prisma.product.create({
      data: {
        ...
        prices:
          typeof data.prices === 'object' && data.prices !== null
            ? data.prices
            : { EUR: 0 },
        affiliate_url: data.affiliate_url || '',
        eco_score:
          typeof data.eco_score === 'number' ? data.eco_score : null,
        ai_confidence:
          typeof data.ai_confidence === 'number' ? data.ai_confidence : null,
        confidence_pct:
          typeof data.confidence_pct === 'number'
            ? data.confidence_pct
            : null,
        confidence_color:
          ConfidenceColor[data.confidence_color as keyof typeof ConfidenceColor] ||
          ConfidenceColor.yellow,
        verified_status:
          VerifiedStatus[data.verified_status as keyof typeof VerifiedStatus] ||
    ...
    res
      .status(400)
      .json({ error: 'Erreur ajout produit', details: (error as Error).message });
  }
});
